	...
	public SpellContext shouldCastThat(Status status, Wizard wizard, Entities entities) {
		if(wizard.isAttacking()) {
			if(status.getMagic()>30) {
				FlipendoStatus spellStatus = status.get(FlipendoStatus.class);
				...
				return snaffles.stream()
					.filter(s -> {
						...
						return !expected.intersectsWith(wizard.getAttackedGoal()) && wizard.getAttackedGoal().intersectsWith((Line) expected);
					})
